...
SFO.fit <- mkinfit(SFO, FOCUS_2006_C, lower = c(k_parent_sink = 0))
summary(SFO.fit)
SFORB.fit <- mkinfit(SFORB, FOCUS_2006_C, plot=TRUE, eigen=FALSE)
summary(SFORB.fit)
SFO_SFO.fit <- mkinfit(SFO_SFO, FOCUS_2006_D, plot=TRUE)
SFO_SFO.fit.ode <- mkinfit(SFO_SFO, FOCUS_2006_D, plot=TRUE, eigen=FALSE)
summary(SFO_SFO.fit, data=FALSE)
summary(SFO_SFO.fit.ode, data=FALSE)
